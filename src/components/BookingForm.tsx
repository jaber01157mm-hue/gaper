...
import { Calendar, Clock, Car, ChevronRight, CheckCircle2, Loader2, Sparkles, ChevronDown } from 'lucide-react';
...
export default function BookingForm() {
  ...
  const [makes, setMakes] = useState<string[]>(['Toyota', 'Honda', 'BMW', 'Mercedes-Benz', 'Ford', 'Chevrolet', 'Nissan', 'Hyundai', 'Kia', 'Volkswagen', 'Audi', 'Lexus']);
  const [models, setModels] = useState<string[]>([]);
  const [isLoadingMakes, setIsLoadingMakes] = useState(false);
  const [isLoadingModels, setIsLoadingModels] = useState(false);

  // Fetch all car makes on mount
  useEffect(() => {
    const fetchMakes = async () => {
      ...
      try {
        ...
        const makeNames = Array.from(new Set(data.Results.map((item: any) => item.MakeName)))
          .sort() as string[];
        if (makeNames.length > 0) setMakes(makeNames);
      } catch (error) {
  ...
  // Fetch models whenever vehicleMake changes
  useEffect(() => {
    const fetchModels = async () => {
      ...
      try {
        const response = await fetch(`https://vpic.nhtsa.dot.gov/api/vehicles/GetModelsForMake/${formData.vehicleMake}?format=json`);
        const data = await response.json();
        const modelNames = Array.from(new Set(data.Results.map((item: any) => item.Model_Name)))
          .sort() as string[];
    ...
    // Small delay to avoid API spam while typing
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setIsSuccess(true);
      setFormData({ vehicleMake: '', vehicleModel: '', serviceType: 'oil_change', date: '', time: '', notes: '' } as any);
    } catch (error) {
                ...
                >
                  <div className="space-y-6">
                    <div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="relative">
                          <Car className={`absolute left-4 top-1/2 -translate-y-1/2 ${isLoadingMakes ? 'text-[#F27D26] animate-pulse' : 'text-white/20'}`} size={18} />
                          <select
                            required
                            className="w-full bg-black/40 border border-white/10 rounded-xl py-3 pl-12 pr-10 focus:ring-2 focus:ring-[#F27D26] outline-none appearance-none cursor-pointer transition-all"
                            value={formData.vehicleMake}
                            onChange={e => setFormData({ ...formData, vehicleMake: e.target.value, vehicleModel: '' })}
                          >
                            <option value="" disabled>{isLoadingMakes ? "Loading..." : "Choose Make"}</option>
                            {makes.map(make => (
                              <option key={make} value={make} className="bg-[#1a1a1a]">{make}</option>
                            ))}
                          </select>
                        ...
                        <div className="relative">
                          <select
                            ...
                            className="w-full bg-black/40 border border-white/10 rounded-xl py-3 px-4 pr-10 focus:ring-2 focus:ring-[#F27D26] outline-none appearance-none cursor-pointer transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                            value={formData.vehicleModel}
                            onChange={e => setFormData({ ...formData, vehicleModel: e.target.value })}
                          >
                            <option value="" disabled>{isLoadingModels ? "Loading models..." : "Choose Model"}</option>
                            {models.map(model => (
                              <option key={model} value={model} className="bg-[#1a1a1a]">{model}</option>
                            ))}
                          </select>
                          <ChevronDown className={`absolute right-4 top-1/2 -translate-y-1/2 text-white/20 pointer-events-none ${isLoadingModels ? 'hidden' : ''}`} size={16} />
                          {isLoadingModels && (
                            <Loader2 className="absolute right-4 top-1/2 -translate-y-1/2 text-[#F27D26] animate-spin" size={14} />
                          )}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="relative">
                      ...
                      <select
                        className="w-full bg-black/40 border border-white/10 rounded-xl py-3 px-4 pr-10 focus:ring-2 focus:ring-[#F27D26] outline-none appearance-none cursor-pointer transition-all"
                        value={formData.serviceType}
                        onChange={e => setFormData({ ...formData, serviceType: e.target.value as ServiceType })}
                      >
                        <option value="oil_change" className="bg-[#1a1a1a]">Oil Change Service</option>
                        <option value="mechanical" className="bg-[#1a1a1a]">Mechanical Repair</option>
                        <option value="electrical" className="bg-[#1a1a1a]">Electrical & Diagnostics</option>
                        <option value="diagnostic" className="bg-[#1a1a1a]">Computer Scan</option>
                      </select>
                      <ChevronDown className="absolute right-4 top-[70%] -translate-y-1/2 text-white/20 pointer-events-none" size={16} />
                    </div>
                  </div>
